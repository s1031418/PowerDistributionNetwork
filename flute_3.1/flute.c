...
void gettree(Tree t , std::vector<node> & SteinerTree)
{
    int i;
    ...
    for (i=0; i<t.deg; i++)
    {
		//bool exist = false ; 
        node subnode ;
        subnode.isSteiner = false;
        subnode.x = (float) t.branch[i].x ;
        subnode.y = (float) t.branch[i].y ;
        subnode.target = t.branch[i].n ;
	//	for(int j = 0 ; j < SteinerTree.size() ; j++)
	//	{
	//		if( SteinerTree[j].x == subnode.x && SteinerTree[j].y == subnode.y )
	//			exist = true ;
	//	}
    //    if( !exist )SteinerTree.push_back(subnode);
		SteinerTree.push_back(subnode);
    }
    for (i=t.deg; i<2*t.deg-2; i++)
    {
		//bool exist = false ; 
        node subnode ;
        subnode.isSteiner = true;
        subnode.x = (float) t.branch[i].x ;
        subnode.y = (float) t.branch[i].y ;
        subnode.target = t.branch[i].n ;
//		for(int j = 0 ; j < SteinerTree.size() ; j++)
