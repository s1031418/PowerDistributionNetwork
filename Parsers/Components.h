...
class Line
{
    ...
    {
        if ( this->pt1.x > right.pt1.x ) //use pt1.x 1 highest 
            return true;
        else if ( this->pt1.x < right.pt1.x )
            return false;
        else 
        {
            if (this->pt1.y > right.pt1.y) //pt1.y 2 highest 
                return true ;
            else if (this->pt1.y < right.pt1.y)
                return false;
            else 
            {
                if (this->pt2.x > right.pt2.x) //pt2.x 3 highest 
                    return true;
                else if (this->pt2.x < right.pt2.x)
                    return false;
                else 
                {
                    if (this->pt2.y > right.pt2.y) //pt2.y 4 highest 
                        return true;
                    else if (this->pt2.y < right.pt2.y)
                        return false;
                    else  //metal name last 
                        return this->MetalName < right.MetalName ;
                }
            }
        }
