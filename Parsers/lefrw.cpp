...
int isSessionles = 0;
std::map<std::string,Via> ViaMaps ;
std::map<std::string,Layer> LayerMaps;
std::map<std::string,Macro> MacroMaps;
Macro mymacro ;
...
//                            via->lefiVia::xl(i, j), via->lefiVia::yl(i, j),
//                            via->lefiVia::xh(i, j), via->lefiVia::yh(i, j));
                    innerlayer.pt1.x = via->lefiVia::xl(i, j);
                    innerlayer.pt1.y = via->lefiVia::yl(i, j);
                    innerlayer.pt2.x = via->lefiVia::xh(i, j);
                    innerlayer.pt2.y = via->lefiVia::yh(i, j);
//                    printf("x1:%f\n" , innerlayer.pt1.x);
//                    printf("y1:%f\n" , innerlayer.pt1.y);
//                    printf("x2:%f\n" , innerlayer.pt2.x);
//                    printf("y2:%f\n" , innerlayer.pt2.y);
    ...
    ViaMaps.insert(std::make_pair(VIA.NAME, VIA));
//    fprintf(fout, "END %s\n", via->lefiVia::name());
//    lefrw::LayerMaps;
//    ViaMaps.insert(std::make_pair(VIA.NAME, VIA));
...
// 0 for obs innerlayer , 1 for pin
std::map<std::string , InnerLayer> prtGeometry(lefiGeometries *geometry  ) {
    ...
    std::map<std::string , InnerLayer> innerlayerMaps;
    InnerLayer innerlayer ;
    for (i = 0; i < numItems; i++) {
        ...
        switch (geometry->lefiGeometries::itemType(i)) {
            case  lefiGeomClassE:
                ...
                break;
            case lefiGeomLayerE:
                innerlayer.NAME = geometry->lefiGeometries::getLayer(i) ;
//                fprintf(fout, "      LAYER %s ;\n",
//                        geometry->lefiGeometries::getLayer(i));
            ...
            case lefiGeomRectE:
                ...
                if (rect->colorMask != 0) {
                    ...
                    innerlayer.pt1.x = rect->xl ;
                    innerlayer.pt1.y = rect->yl ;
                    innerlayer.pt2.x = rect->xh ;
                    innerlayer.pt2.y = rect->yh ;
//                    fprintf(fout, "      RECT ( %f %f ) ( %f %f ) ;\n", rect->xl,
//                            rect->yl, rect->xh, rect->yh);
                }
                break;
        ...
        if( (i + 1 ) % 2 == 0 )innerlayerMaps.insert(std::make_pair(innerlayer.NAME, innerlayer));
        
    }
    return innerlayerMaps ;
}
...
int layerCB(lefrCallbackType_e c, lefiLayer* layer, lefiUserData ud) {
    ...
    mylayer.NAME = layer->lefiLayer::name();
//    fprintf(fout, "LAYER %s\n", layer->lefiLayer::name());
    if (layer->lefiLayer::hasType())
    {
        mylayer.TYPE = layer->lefiLayer::type() ;
//        fprintf(fout, "  TYPE %s ;\n", layer->lefiLayer::type());
    ...
    {
        mylayer.PITCH_DISTANCE = layer->lefiLayer::pitch() ;
//        fprintf(fout, "  PITCH %g ;\n", layer->lefiLayer::pitch());
    ...
    {
        mylayer.OFFSET_DISTANCE = layer->lefiLayer::offset() ;
//        fprintf(fout, "  OFFSET %g ;\n", layer->lefiLayer::offset());
    ...
    {
        mylayer.WIDTH = layer->lefiLayer::width() ;
//        fprintf(fout, "  WIDTH %g ;\n", layer->lefiLayer::width());
    ...
    if (layer->lefiLayer::hasMaxwidth()) {
        mylayer.MAXWIDTH = layer->lefiLayer::maxwidth();
//        fprintf(fout, "  MAXWIDTH %g ;\n", layer->lefiLayer::maxwidth());
    ...
    if (layer->lefiLayer::hasSpacingNumber()) {
        for (i = 0; i < layer->lefiLayer::numSpacing(); i++) {
            mylayer.SPACING = layer->lefiLayer::spacing(i) ;
//            fprintf(fout, "  SPACING %g ", layer->lefiLayer::spacing(i));
    ...
    {
        mylayer.DIRECTION = layer->lefiLayer::direction() ;
//        fprintf(fout, "  DIRECTION %s ;\n", layer->lefiLayer::direction());
    }

    if (layer->lefiLayer::hasResistance())
    {
        mylayer.RESISTANCE_RPERSQ = layer->lefiLayer::resistance() ;
...
//    fprintf(fout, "END %s\n", layer->lefiLayer::name());
    LayerMaps.insert(std::make_pair(mylayer.NAME, mylayer));
...
int macroCB(lefrCallbackType_e c, lefiMacro* macro, lefiUserData ud) {
    ...
    {
        mymacro.CLASS = macro->lefiMacro::macroClass() ;
    }
...
//    }
    ...
    {
        mymacro.WIDTH = macro->lefiMacro::sizeX() ;
        mymacro.LENGTH = macro->lefiMacro::sizeY() ;
...
//                    macro->lefiMacro::sizeY());
    ...
    {
        mymacro.ORIGINX = macro->lefiMacro::originX() ;
        mymacro.ORIGINY = macro->lefiMacro::originY() ; 
    }
...
//    fprintf(fout, "  OBS\n");
    ...
    mymacro.obs.InnerMaps = prtGeometry(geometry);
...
//    fprintf(fout, "  PIN %s\n", pin->lefiPin::name());
    ...
    {
        mypin.DIRECTION = pin->lefiPin::direction() ;
//        fprintf(fout, "    DIRECTION %s ;\n", pin->lefiPin::direction());
    }
    if (pin->lefiPin::hasUse())
    {
        mypin.USE = pin->lefiPin::use() ;
//        fprintf(fout, "    USE %s ;\n", pin->lefiPin::use());
    }
...
//        fprintf(fout,"    PORT\n");
        ...
        mypin.InnerMaps = prtGeometry(geometry);
//        fprintf(fout, "    END\n");
    }
//    fprintf(fout, "  END %s\n", pin->lefiPin::name());
...
//        lefrSetWarningLogFunction(printWarning);
        ...
        lefrSetMacroBeginCbk(macroBeginCB); // use
        ...
        lefrSetObstructionCbk(obstructionCB); // use
        lefrSetPinCbk(pinCB); // use
